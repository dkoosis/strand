...
func (s *Server) insightsModel(ctx context.Context, src readSource, v *listView, issues []bd.Issue) (insights, error) {
	...
	out.Ready = readyQueue(beads, deps, idx, m.PageRank, s.now())
	...
	if len(compEdges) > 0 {
		out.Influence = crossFlag(leaderboard(beads, m.PageRank), deps, idx, s.now())
		out.Bottleneck = crossFlag(leaderboard(beads, m.Betweenness), deps, idx, s.now())
	}
	return out, nil
}
...
func readyQueue(beads []forest.Bead, deps []bd.DepEdge, idx map[string]bd.Issue, score map[string]float64, now time.Time) []rankedBead {
	openBlockers := blockerCounts(deps, idx)
...
func crossFlag(board []rankedBead, deps []bd.DepEdge, idx map[string]bd.Issue, now time.Time) []rankedBead {
	openBlockers := blockerCounts(deps, idx)
	for i := range board {
		id := board[i].ID
