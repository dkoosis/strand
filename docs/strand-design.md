...
## 1a. The synthesis reframe `[settled 2026-06-21]`
...
complete **catalog**. What's missing is **synthesis** — the few things in motion, their
shape, health, and drift, rendered so a human grasps the whole in one look.

**Synthesis is not a shorter catalog.** Collapsing the task list to its top rows yields a
smaller catalog, not synthesis. Synthesis is a *different artifact*: aggregation and
roll-up over the catalog. This is strand's entire reason to exist, and it is probably
**not a table** (a table is a good catalog, poor synthesis).
...
- **THEN** the system renders the dependency graph beneath it with status coloring
